...
export class SystemCommandCenter extends LitElement {
  ...
  @state() private nodes: NodeStatus[] = [
    { id: 1, region: 'Research copilot · eu-west', load: 42, state: 'stable' },
    { id: 2, region: 'Support agent · us-east', load: 78, state: 'watch' },
    { id: 3, region: 'Commerce agent · ap-south', load: 51, state: 'stable' },
  ]
  ...
  render() {
    ...
    return html`<header><div><span class="eyebrow">Agent observability components</span><h1>Agent telemetry observatory</h1><p>Composable custom elements with encapsulated styles and reactive controllers.</p></div><span class="pulse"><i></i>${this.pulse.label}</span></header>
      <section class="metrics"><status-card label="Tool pressure" value="${average}%" detail="Across all regions"></status-card><status-card label="Active agents" value="${this.nodes.length}" detail="Shadow DOM islands"></status-card><status-card label="Trace pulse" value="${this.pulse.tick}" detail="Reactive controller"></status-card></section>
      <section class="panel"><div class="title"><div><span class="eyebrow">Agent runtime</span><h2>Agent workflows</h2></div><span>Updated ${this.pulse.label}</span></div>
        ${repeat(this.nodes, (node) => node.id, (node) => html`<article><span class="state ${node.state}"></span><div><b>${node.region}</b><small>${node.state}</small></div><div class="meter"><i style="width:${node.load}%"></i></div><strong>${node.load}%</strong><button @click=${() => this.rebalance(node.id)} ?disabled=${node.load <= 30}>Intervene</button></article>`)}
      </section>`
  }
